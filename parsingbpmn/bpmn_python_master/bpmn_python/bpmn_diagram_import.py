...
class BpmnDiagramGraphImport(object):
    ...
    def import_process_elements(document, diagram_graph, sequence_flows, process_elements_dict, plane_element):
        ...
        for process_element in document.getElementsByTagNameNS("*", consts.Consts.process):
            ...
            targetref_dataoutputassociation=""
            ...
            for flow in utils.BpmnImportUtils.iterate_elements(process_element):
                if flow.nodeType != flow.TEXT_NODE:
                    ...
                    """
                    if(flow.childNodes):
                        for childnode in flow.childNodes:
                            if(childnode.nodeType != flow.TEXT_NODE):
                               if(childnode.tagName== "bpmn:"+consts.Consts.dataOutputAssociation):
                                   id_dataoutputassociation=childnode.getAttribute(consts.Consts.id)
                                   for childnodeTargetRef in childnode.childNodes:
                                       if (childnodeTargetRef.nodeType != flow.TEXT_NODE):
                                           if (childnodeTargetRef.tagName == "bpmn:" + consts.Consts.target_ref):
                                                targetref_dataoutputassociation=childnodeTargetRef.firstChild.data
                               #print(childnode)
                    """
    ...
    @staticmethod
    def __import_element_by_tag_name(diagram_graph, sequence_flows, process_id, process_attributes, element, tag_name,plane_element):
        if tag_name == consts.Consts.task \
            ...
            BpmnDiagramGraphImport.import_boundary_event_to_graph(diagram_graph, process_id, process_attributes,
                                                                  element)
        elif tag_name == consts.Consts.association:
            BpmnDiagramGraphImport.import_association(diagram_graph, process_id, process_attributes,
                                                      element)
    ...
    @staticmethod
    def import_flow_node_to_graph(bpmn_graph, process_id, process_attributes, flow_node_element,plane_element):
        """
        ...
        # dataOutput = flow_node_element.getAttribute(consts.Consts.dataOutputAssociation)
        bpmn_graph.add_node(element_id)
        # bpmn_graph.add_node(dataOutput)
        bpmn_graph.nodes[element_id][consts.Consts.id] = element_id
        # bpmn_graph.nodes[dataOutput][consts.Consts.dataOutputAssociation] = dataOutput
        ...
        for tmp_element in utils.BpmnImportUtils.iterate_elements(flow_node_element):
            if tmp_element.nodeType != tmp_element.TEXT_NODE:
                ...
                if tag_name == consts.Consts.dataOutputAssociation:
                    ...
                    #if(consts.Consts.id not in bpmn_graph.nodes[element_id][consts.Consts.dataOutputAssociation].keys()):
                        #bpmn_graph.nodes[element_id][consts.Consts.dataOutputAssociation][consts.Consts.id] = {}
                    #if(consts.Consts.target_ref not  in bpmn_graph.nodes[element_id][consts.Consts.dataOutputAssociation].keys()):
                        #bpmn_graph.nodes[element_id][consts.Consts.dataOutputAssociation][consts.Consts.target_ref] = {}
                    #if(consts.Consts.targetX not  in bpmn_graph.nodes[element_id][consts.Consts.dataOutputAssociation].keys()):
                        #bpmn_graph.nodes[element_id][consts.Consts.dataOutputAssociation][consts.Consts.targetX] = {}
                    #if(consts.Consts.targetY not  in bpmn_graph.nodes[element_id][consts.Consts.dataOutputAssociation].keys()):
                        #bpmn_graph.nodes[element_id][consts.Consts.dataOutputAssociation][consts.Consts.targetY] = {}
                    ...
                    for child in tmp_element.childNodes:
                        if type(child) is Element:
                            ...
                            data_child = (text)
                            temp[consts.Consts.id] = (data_output)
                        ...
                        if element.nodeType != element.TEXT_NODE:
                            ...
                            if tag_name == consts.Consts.bpmn_edge:
                                if(element.getAttribute("bpmnElement") == temp[consts.Consts.id]):
                                    ...
                                    #bpmn_graph.nodes[element_id][consts.Consts.dataOutputAssociation][consts.Consts.targetX] =waypoints[1][0]
                                    #bpmn_graph.nodes[element_id][consts.Consts.dataOutputAssociation][consts.Consts.targetY] = waypoints[1][1]
    ...
    def import_task_to_graph(diagram_graph, process_id, process_attributes, task_element, plane_element):
        ...
        """
        BpmnDiagramGraphImport.import_activity_to_graph(diagram_graph, process_id, process_attributes, task_element,plane_element)
        ...
        for element in utils.BpmnImportUtils.iterate_elements(subprocess_element):
            if element.nodeType != element.TEXT_NODE:
                ...
                BpmnDiagramGraphImport.__import_element_by_tag_name(diagram_graph, sequence_flows, subprocess_id,
                                                                    subprocess_attributes, element, tag_name,None)
    ...
    def import_data_object_reference_to_graph(diagram_graph, process_id, process_attributes, data_object_element):
        ...
        :param data_object_element: object representing a BPMN XML 'dataObject' element.
        """
        # data_object_element.getAttribute(consts.Consts.dataObjectRef)

        BpmnDiagramGraphImport.import_flow_node_to_graph(diagram_graph, process_id, process_attributes,
                                                         data_object_element,None)
        data_object_id = data_object_element.getAttribute(consts.Consts.id)
        diagram_graph.nodes[data_object_id][consts.Consts.dataObjectRef] = \
            data_object_element.getAttribute(consts.Consts.dataObjectRef)
    ...
    def import_activity_to_graph(diagram_graph, process_id, process_attributes, element,plane_element):
        ...
        """
        BpmnDiagramGraphImport.import_flow_node_to_graph(diagram_graph, process_id, process_attributes, element,plane_element)
    ...
    def import_gateway_to_graph(diagram_graph, process_id, process_attributes, element):
        ...
        """
        element_id = element.getAttribute(consts.Consts.id)
        BpmnDiagramGraphImport.import_flow_node_to_graph(diagram_graph, process_id, process_attributes, element,None)
    ...
    def import_association(cls, diagram_graph, process_id, process_attributes, element):
        ...
        if type(element) is Element:
            ...
            id = list(element.attributes.values())[0].value
            sourceRef = list(element.attributes.values())[1].value
            targetRef = list(element.attributes.values())[2].value
            assoc.append(id)
